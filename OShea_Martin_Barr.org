...
src_R{round(p2eta(.8, .m1), 2)} or smaller on the log odds scale, corresponding to a difference in misspecification rate of about
...
src_R{round(p2eta(.9, .m1), 2)} or smaller on the log odds scale, or a difference in response rate of 
...
src_R{round(p2eta(.95, .m1), 2)} or smaller on the log odds scale, or a difference in response rate of 
...
src_R{sprintf("\\(\\Chi^2(1) = %0.2f\\)", d_rslp_chi_s)},
src_R{sprintf("\\(p = %0.3f\\)", pchisq(d_rslp_chi_s, 1, lower.tail = FALSE))}. (We did not test the significance of the by-item random slope because the model estimated this parameter at zero.)
...
#+CAPTION: Sensitivity plot for Experiment 3. Points reflect the proportion of significant tests at each of the six effect sizes examined; line is a fit from a logistic regression model to allow smooth interpolation.
#+LABEL: fig:exp3-sens
#+RESULTS: fig:exp3-sens
[[file:exp3/img/exp3-sens.pdf]]

We also peformed a sensitivity test for the analysis of the main effect of /Visible Partner/, following the same procedure described for Experiment\nbsp{}1, with the difference that the effect size range we examined was from .1 to .5. This was also not a pre-registered analysis. Results are shown in Figure\nbsp{}[[fig:exp3-sens]]. 
The simulations suggest our analysis had: (1) 80% power to reject an effect of 
src_R{round(p2eta(.8, .m1), 2)} or smaller on the log odds scale, corresponding to a difference in misspecification rate of about
src_R{as.perc(.rge_80["diff"])} between conditions (e.g., 
src_R{as.perc(.rge_80["low"])} in the high distortion condition versus
