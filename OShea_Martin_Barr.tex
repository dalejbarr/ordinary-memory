...
\label{sec:org4cb1324}

\begin{figure}[htbp]
\centering
\includegraphics[width=.9\linewidth]{figs/Exp1.png}
\caption{\label{fig:org4658532}
...
The data are shown in Figure~\ref{fig:org97bb536}. Parameters were estimated using the \texttt{lmer()} function under maximum likelihood with identity link and Gaussian variance. The dependent variable was the speech latency for the test trial minus the speech latency for the final training trial for that sequence; in other words, the change in speech latency incurred by abandoning the entrained description. We tested against the null using a two-tailed test on the Wald \(z\) statistic with \(\alpha = .05\).  
